...
void merge_blocksnp()
{
        ...
        for(a5=0; a5<p; a5++)
        {
            if((a5%pt)!=0)
            {
                for(a6=1; a6<k; a6++)
                {
                    ...
                    merge_key_set[merge_k][a6+k-1][0]=key_set[(a4+1)*p+(a5-1)][a6][0];
                    merge_key_set[merge_k][a6+k-1][1]=key_set[(a4+1)*p+(a5-1)][a6][1];
            ...
            if((a5%pt)==0)
            {
                for(a7=1; a7<k; a7++)
                {
                    ...
                    merge_key_set[merge_k][a7+k-1][0]=key_set[(a4+1)*p+(a5+(pt-1))][a7][0];
                    merge_key_set[merge_k][a7+k-1][1]=key_set[(a4+1)*p+(a5+(pt-1))][a7][1];
    ...
    for(a8=0; a8<p-4; a8=a8+2)                               //handling last 3 rows without last 3X3   0 to p-4
    {
        for(a9=1; a9<k; a9++)
        {
            ...
            merge_key_set[merge_k][a9+k-1][0]=key_set[(p-1)*p+(a8+1)][a9][0];
            merge_key_set[merge_k][a9+k-1][1]=key_set[(p-1)*p+(a8+1)][a9][1];
        ...
        for(a10=1; a10<k; a10++)
        {
            ...
            merge_key_set[merge_k][a10+k-1][0]=key_set[(p-3)*p+(a8+1)][a10][0];
            merge_key_set[merge_k][a10+k-1][1]=key_set[(p-3)*p+(a8+1)][a10][1];
        ...
        for(a11=1; a11<k; a11++)
        {
            ...
            merge_key_set[merge_k][a11+k-1][0]=key_set[(p-1)*p+a8][a11][0];
            merge_key_set[merge_k][a11+k-1][1]=key_set[(p-1)*p+a8][a11][1];
    ...
    for(a9=1; a9<k; a9++)
    {
        ...
        merge_key_set[merge_k][a9+k-1][0]=key_set[(p-1)*p+(p-2)][a9][0];
        merge_key_set[merge_k][a9+k-1][1]=key_set[(p-1)*p+(p-2)][a9][1];
    }
    merge_k++;
    ...
    for(a9=1; a9<k; a9++)
    {
        ...
        merge_key_set[merge_k][a9+k-1][0]=key_set[(p-2)*p+(p-3)][a9][0];
        merge_key_set[merge_k][a9+k-1][1]=key_set[(p-2)*p+(p-3)][a9][1];
    }
    merge_k++;
    ...
    for(a9=1; a9<k; a9++)
    {
        ...
        merge_key_set[merge_k][a9+k-1][0]=key_set[(p-2)*p+(p-2)][a9][0];
        merge_key_set[merge_k][a9+k-1][1]=key_set[(p-2)*p+(p-2)][a9][1];
    }
    merge_k++;
    ...
    for(a9=1; a9<k; a9++)
    {
        ...
        merge_key_set[merge_k][a9+k-1][0]=key_set[(p-1)*p+(p-3)][a9][0];
        merge_key_set[merge_k][a9+k-1][1]=key_set[(p-1)*p+(p-3)][a9][1];
    }
    merge_k++;
...
//print keys
    ...
    merge_blocksnp();  // merging procedure call
   ...
   // printf("\ncompromised keys::");
    ...
    for(a1=0; a1<merge_node; a1++)
    {
        ...
        for(a2=0; a2<merge_node; a2++)
        {
            ...
            count2=0;
            for(a3=1; a3<2*k-1; a3++)
            {
                for(a4=1; a4<2*k-1; a4++)
                {
    ...
    fprintf(fp,"\nbroken links: %d\n",flag_broken);
    float total_links;
    total_links=merge_node*(merge_node-1)*0.5;
