...
int merge_key_set[5100][200][2],merge_key_set_copy[1000][200][2];  //merging bblock key set and copy set
int comp1,comp2;
int x;
...
//merging block
    ...
    if(qrs%2!=0)                                            //if qrs is odd
    {
        ...
        merge_set[a3][0]=(qrs-3)*qrs+qrs-3;
        merge_set[a3][1]=(qrs-2)*qrs+qrs-2;
        a3++;

        merge_set[a3][0]=(qrs-3)*qrs+qrs-2;
        merge_set[a3][1]=(qrs-2)*qrs+qrs-1;
        a3++;

        merge_set[a3][0]=(qrs-2)*qrs+qrs-3;
        merge_set[a3][1]=(qrs-1)*qrs+qrs-2;
        a3++;

        merge_set[a3][0]=(qrs-1)*qrs+qrs-3;
        merge_set[a3][1]=(qrs-3)*qrs+qrs-1;
    }
    else
    {
        a3=0;
        z_c=0;
        for(a1=0; a1<qrs-1; a1=a1+2)                        //taking 2 rows at a time
        {
            for(a2=0; a2<qrs-1; a2++)                       //iteration will go from 0 rro qrs-2
            {
                merge_set[a3][0]=a1*qrs+a2;                 //merging diagonal elements
    ...
    printf("\nbroken links: %d\n",flag_broken);
    fprintf(fp3,"\n----START OF RECORD---");
    fprintf(fp3,"\nTotal nodes in the n/w: %d",no_of_nodes);
    fprintf(fp3,"\n Merged nodes in the n/w: %d",node);
    fprintf(fp3,"\n %d no of nodes are merged",z);
    fprintf(fp3,"\nKeys per node: %d",keys_per_node);
    fprintf(fp3,"\nCompromised nodes: %d",compromised_node);
    fprintf(fp3,"\nbroken links: %d\n",flag_broken);
    float total_links;
    total_links=(node*(node-1))/2;
