...
int arr_node[3000];//random nodes array
int isprime(int x)
{
    int a,a1,count;
    count=0;
...
void main()
{
    ...
    int compromised[100],compromised_keys[2000][2],count1=0,common_key[2000][2],count2=0,flag2=0;
    int flag_broken=0,node_list[3000],node_status[3000];
    int qt;
    int merge_block[1000][4];
    int flag_vs=0,flag_c=0;
    FILE* fp;
...
//merging block
    ...
    for(a1=0; a1<merge_node; a1++)
    {
        ...
        {
            if(a8==a2 || a8==a3 || node_status[a8]==0)
                continue;
            for(a4=0; a4<2*keys_per_node; a4++)
            {
                for(a5=0; a5<keys_per_node; a5++)
                {
                    if(merge_key_set[a1][a4][0]==key_set[a8][a5][0] && merge_key_set[a1][a4][1]==key_set[a8][a5][1])
                    {
                        node_status[a8]=0;
                        flag_n2=1;
                        break;
        ...
        for(a9=a7; a9<3*keys_per_node; a9++)
        {
            merge_key_set[a1][a9][0]=key_set[a3][a9-a7][0];
            merge_key_set[a1][a9][1]=key_set[a3][a9-a7][1];
        }

        merge_block[a1][2]=a8;

        //4th
        for(a10=0; a10<no_of_nodes; a10++)
        {
            if(a10==a2 || a10==a3 || a10==a8 || node_status[a10]==0)
                continue;
            for(a4=0; a4<3*keys_per_node; a4++)
            {
                for(a5=0; a5<keys_per_node; a5++)
                {
                    if(merge_key_set[a1][a4][0]==key_set[a10][a5][0] && merge_key_set[a1][a4][1]==key_set[a10][a5][1])
                    {
                        node_status[a10]=0;
                        flag_n3=1;
                        break;
        ...
        for(a11=a9; a11<4*keys_per_node; a11++)
        {
            merge_key_set[a1][a11][0]=key_set[a3][a11-a9][0];
            merge_key_set[a1][a11][1]=key_set[a3][a11-a9][1];
        }
        merge_block[a1][3]=a10;
    }
...
//checking links
    ...
    flag_broken=flag_broken/6;  //factor 2 to eliminate w way counting and 3 to triple counting in flag stage
    printf("\nbroken links: %d\n",flag_broken);
    fprintf(fp,"\nbroken links: %d\n",flag_broken);
    int total_links;
