...
int merge_key_set[5100][200][2],merge_key_set_copy[5100][200][2];  //merging bblock key set and copy set
int comp1,comp2;
int x;
...
//merging block
    ...
    {
        a3=0;
        for(a1=0; a1<qrs;a1++)                           //horizontal grouping
        {
            for(a2=0;a2<qrs-2;a2=a2+2)
            {
                merge_set[a3][0]=a1*qrs+a2;
                merge_set[a3][1]=a1*qrs+(a2+1);
                fprintf(fp1,"\n<(%d,%d) (%d,%d)>   %d",merge_set[a3][0]/qrs,merge_set[a3][0]%qrs,merge_set[a3][1]/qrs,merge_set[a3][1]%qrs,a3);
                a3++;
            }
            fprintf(fp1,"\n");
        }
        for(a1=0;a1<qrs-2;a1=a1+2)                          //vertical grouping
        {
            merge_set[a3][0]=a1*qrs+(qrs-1);
            merge_set[a3][1]=(a1+1)*qrs+(qrs-1);
            fprintf(fp1,"\n<(%d,%d) (%d,%d)>   %d",merge_set[a3][0]/qrs,merge_set[a3][0]%qrs,merge_set[a3][1]/qrs,merge_set[a3][1]%qrs,a3);
            a3++;
        }
    }
